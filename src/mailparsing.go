 ...
 * inlyse-pilot-api
 * ~~~~~~~~~~~~~~~~~
* copyright: (c) 2020 by inlyse. All rights reserved. See AUTHORS for more details.
* license: Closed Source, see LICENSE for more details.
*/
...
func Parse(r io.Reader) (email Email, err error) {
	...
	email, err = mail_parser(msg.Header)
	if err != nil {
		return
	}
	...
	email.ContentTypeParsed = contentType
	if err != nil {
		return
	}
	...
	email.Content_Transfer_Encoding = msg.Header.Get("Content-Transfer-Ecoding")
	return
}

func mail_parser(header mail.Header) (email Email, err error) {
	hp := headerParser{header: &header}

	email.Received_SPF = header.Get("Received-Spf")
	email.XMailer = header.Get("X-Mailer")
	//email.DKIM = header.Get
	...
	email.ReplyTo = hp.parseAddress(header.Get("Reply-To"))
	...
	email.ReturnPath = hp.parseAddress(header.Get("Return-Path"))
